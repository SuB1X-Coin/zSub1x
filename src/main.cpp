...
#if defined(NDEBUG)
#error "zSub1x cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from zSub1x
...
{
    RenameThread("zsub1x-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZSUB1XMinted()
{
...
void RecalculateZSUB1XSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSUB1X supply
...
bool RecalculateSUB1XSupply(int nHeightStart)
{
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SUB1X to zSUB1X
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // zSub1x: We use certain sporks during IBD, so check to see if they are
