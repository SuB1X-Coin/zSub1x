...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a zSub1x address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no zSub1x: URI
...
{
    // Convert zsub1x:// to zsub1x:
    //
    //    Cannot handle this later, because zsub1x:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open zsub1x.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "zSub1x.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for zSub1x.lnk
...
{
    return GetAutostartDir() / "zsub1x.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a zsub1x.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=zSub1x\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the zsub1x app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add zsub1x app to startup item list
