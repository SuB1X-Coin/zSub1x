...
// Possible states for zSUB1X send
enum ZerocoinSpendStatus {
    ZSUB1X_SPEND_OKAY = 0,                            // No error
    ZSUB1X_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSUB1X_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSUB1X_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSUB1X_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSUB1X_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSUB1X_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSUB1X_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSUB1X_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSUB1X_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSUB1X_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSUB1X_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSUB1X_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSUB1X_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSUB1X_SPENT_USED_ZSUB1X = 14                       // Coin has already been spend
};
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 20 * COIN) continue; // do not count MN-like outputs
    ...
    CAmount GetLockedCredit() const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 20 * COIN) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            }
